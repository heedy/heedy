...
Heedy will then diplay the README in the plugin description dialog (the _i_ button):
...
With that, we restart heedy by exiting it in the terminal with `CTRL+C`, and start the existing database with
...
In particular, all of the content in the `public/static` subfolder will be automatically made accessible to _all_ visitors to the site.
