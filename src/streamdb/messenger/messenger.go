...
type SubscriptionFunction func(*timebatchdb.KeyedDatapoint)
...
func (m *Messenger) Publish(d timebatchdb.KeyedDatapoint,routing string) error {
    if (len(routing)==0) {
        routing = d.Key()
    }
...
func (m *Messenger) SubChannel(router string, chn chan timebatchdb.KeyedDatapoint) (*nats.Subscription,error) {
