...
CREATE TABLE plugins (
	...
	cdb_version VARCHAR,		-- Semver compatible CDB versions
...
-- > "connectordb", semver version of most recent ConnectorDB (optional)
...
type Registry struct {
	...
	ConnectorDBVersion *semver.Version
...
func Open(filename string) (*Registry, error) {
	...
	// And finally, check if there is a CDB version specified
	if v, err = r.Get("registry"); err != nil {
		db.Close()
		return nil, err
	}
	if v != "" {
		cdbv, err := semver.New(v)
		if err != nil {
			db.Close()
			return nil, err
		}
		r.ConnectorDBVersion = cdbv
	}
