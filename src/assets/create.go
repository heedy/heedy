...
// directory, and creates an associated CDB database.
...
func Create(directory string, cfg *Configuration, configFile string) (*Assets, error) {
	...
	if err == nil {
		...
		if len(finfo) > 0 {
			return nil, fmt.Errorf("%s already has files in it. Must be empty to initialize CDB in it", directory)
		}
	}
	...
	err = CopyDir(builtinFs, "/newdb", osFs, directory)
	if err != nil {
		return nil, err
	}

	configFilePath := path.Join(directory, "connectordb.conf")
