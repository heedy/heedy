...
class App(APIObject):
    props = {"name","description","icon","meta","settings","settings_schema"}
    ...
    @property
    def owner(self):
        return self.session.f(self.read(),lambda x: users.User(x["owner"],self.session))
...
class Apps(APIList):
    ...
    def __call__(self,**kwargs):
        return self._call(f=lambda x :  [App(xx["id"],session=self.session) for xx in x],**kwargs)

    def create(self,name,otype="stream",**kwargs):
        return self._create(f= lambda x :  App(x["id"],session=self.session) ,**{'name': name,'type': otype, **kwargs})
