...
class SyncSession(Session):
    ...
    def version(self):
        return self.handleResponse(self.s.get(urljoin(self.url,"api/heedy/v1/meta/version"))).text
    def handleResponse(self,r):
        if r.status_code >= 400:
            ...
            try:
                ...
                msg = {"error": "malformed_response", "error_description": f"The server returned \"{r.text}\", which is not json."}
    ...
    async def handleResponse(self,r):
        if r.status >= 400:
            ...
            try:
                ...
                msg = {"error": "malformed_response", "error_description": f"The server did not return valid json"}
            raise HeedyError(msg)
        return r
    async def version(self):
        self.initSession()
        return await (await self.handleResponse(await self.s.get(urljoin(self.url,"api/heedy/v1/meta/version")))).text()
...
class APIObject:
    """
    APIObject represents an object in heedy (user,connection,source,etc).
