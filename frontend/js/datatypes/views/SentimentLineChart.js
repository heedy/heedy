...
const SentimentView = [
  {
    ...
    subtitle: "",
    dropdown: dropdownTransformDisplay(
      "Sentiment is calculated by counting the words with happy/sad connotations using the AFINN-111 wordlist. Sentences deemed positive will have sentiment > 0, while negative sentences will generally have sentiment < 0.",
      "sentiment"
    )
  },
  {
    ...generateDropdownLineChart(
      ...
      generateTimeOptions("Average", "sentiment", "mean"),
      1
    ),
...
function showSentimentChart(context) {
  if (context.data.length > 1) {
    ...
    if (
      hasSentence(context.data[0].d) &&
      hasSentence(context.data[context.data.length - 1].d)
    ) {
