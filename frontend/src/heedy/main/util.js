function filterComponents(components, importance, filter) {
    let cmps = Object.values(components.filter(filter).reduce((o, v) => {
        ...
        // though, since an older key might actually be more specific (for example, a object header
        // for a specific plugin/key combo should not be replaced by a object type header, even if it comes later)
        let newv = {
