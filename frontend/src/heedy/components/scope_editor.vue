  ...
  computed: {
    items() {
      ...
      [{
          ...
          tScopes.map(k => {recognizedSet[k] = true});
          res.push({header: t.description, text: tScopes.join(" ")});
          res = res.concat(tScopes.map(k => ({text: k, description: cscopes[k], color: t.color})));
        });

      let unrecognizedDB = Object.keys(cscopes).filter(v => !(v in recognizedSet)).map(k=> ({text: k, description: cscopes[k], color: "gray"}));
      ...
      if (this.customItems.length > 0 || unrecognizedDB.length > 0) {
        ...
        res.push({header: "Custom & Unrecognized Scopes:", text: unrecognizedDB.map(k=> k.text).join(" ")});
