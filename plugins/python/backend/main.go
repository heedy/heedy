...
import (
	...
	"github.com/heedy/heedy/plugins/python/backend/python"
	"github.com/sirupsen/logrus"
)

func main() {
	logrus.Info(fmt.Sprintf("%s plugin starting", python.PluginName))
	p, err := plugin.Init()
	if err != nil {
		logrus.Error(err)
		os.Exit(1)
	...
	pluginMiddleware := plugin.NewMiddleware(p, python.Handler)

	server := http.Server{
		Handler: pluginMiddleware,
	}

	sockPath := fmt.Sprintf("%s.sock", python.PluginName)
