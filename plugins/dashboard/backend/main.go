...
func main() {
	logrus.Info(fmt.Sprintf("%s plugin starting", dashboard.PluginName))
	p, err := plugin.Init()
	if err != nil {
		logrus.Error(err)
		os.Exit(1)
	}
	err = p.InitSQL(dashboard.PluginName, dashboard.SQLVersion, dashboard.SQLUpdater)
	if err != nil {
		p.Logger().Error(fmt.Errorf("Failed to set up database: %w", err))
		os.Exit(1)
	}
	pluginMiddleware := plugin.NewMiddleware(p, dashboard.Handler)

	server := http.Server{
		Handler: pluginMiddleware,
	}

	sockPath := fmt.Sprintf("%s.sock", dashboard.PluginName)
