...
const DownlinkInput = (state, actions, downlink, path, go) => (
    <DataInput showIcon={true} key={path} size={6} thisUser={state.site.thisUser} thisDevice={state.site.thisDevice} title={downlink.nickname == ""
        ? downlink.name.capitalizeFirstLetter()
        : downlink.nickname} subtitle={path} user={state.site.thisUser} device={{ name: downlink.device, downlink: true }} stream={downlink} icons={[(
            <IconButton key="showstream" onTouchTap={() => go(path)} tooltip="view stream">
...
const Render = ({ state, appstate, actions, go }) => (
    <div style={{
        ...
        {!state.loaded ? (<Loading />) : (state.downlinks.length == 0 ? (<Welcome />) : (
            <div style={{
                marginLeft: "-15px",
                marginRight: "-15px"
            }}>
...
export default connect(
    ...
    (dispatch) => ({ actions: bindActionCreators(Actions, dispatch), go: (v) => dispatch(go(v)) })
)(Render);