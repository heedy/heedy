...
class Storage {
  ...
  addContext(context) {
    ...
    if (context.Timestamp * 1000 > Date.now() - 5000) {
      ...
      if (context.ThisDevice != null) {
        inserter[context.ThisUser.name + "/" + context.ThisDevice.name] =
          context.ThisDevice;
      }
      ...
      if (context.Device != null) {
        inserter[context.User.name + "/" + context.Device.name] =
          context.Device;
      }
  ...
  set(path, obj) {
    ...
    let store = path.split("/").length == 3 ? this.streams : this.store;
  ...
  ls(path) {
    ...
    // Dealing with multiple storage locations
    let store = path.split("/").length == 2 ? this.streams : this.store;
