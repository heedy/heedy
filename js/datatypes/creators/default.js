...
class DefaultRequired extends Component {
    ...
    render() {
        ...
        return (<SchemaEditor value={state.schema} onChange= {(e,schema)=>this.props.setState({schema:schema})}/>);
    }
}
...
class DefaultOptional extends Component {
    ...
    render() {
        ...
        return (
            <div>
                <DatatypeEditor value={state.datatype} schema={state.schema} onChange={(e, txt) => set({datatype: txt})}/>
                <DownlinkEditor value={state.downlink} onChange={(e, txt) => set({downlink: txt})}/>
                <EphemeralEditor value={state.ephemeral} onChange={(e, txt) => set({ephemeral: txt})}/>
            </div>
        );
    }
}
