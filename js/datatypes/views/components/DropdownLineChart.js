...
class DropdownLineChart extends Component {
    static propTypes = {
        ...
        options: PropTypes.arrayOf(PropTypes.shape({description: PropTypes.string, transform: PropTypes.string.isRequired, name: PropTypes.string.isRequired}))
    }
    ...
    render() {
        ...
        return (
            <div style={{
                ...
                <DropDownMenu value={state.currentOption} onChange={(e, i, v) => setState({transform: v.transform, currentOption: v, description: v.description})}>
                    {options.map(function(o) {
                        return (<MenuItem key={o.transform} value={o} primaryText={o.name}/>);
                    })}
                </DropDownMenu>
...
export function generateDropdownLineChart(description, options, defaultOptionIndex) {
    return {
        initialState: {
