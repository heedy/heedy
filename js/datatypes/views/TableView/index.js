    ...
    if (context.state.csv) {
        ...
        if (context.state.allowtime) {
            if (context.state.timeon) {
                resultarray.push((
                    <IconButton key="timer" onTouchTap={() => context.setState({ timeon: false })} tooltip="disble pretty printing duration">
                        <FontIcon className="material-icons" color="rgba(0,0,0,0.8)">
                            alarm_off
                        </FontIcon>
                    </IconButton>
                ...
                resultarray.push((
                    <IconButton key="timer" onTouchTap={() => context.setState({ timeon: true })} tooltip="view data as time duration">
                        <FontIcon className="material-icons" color="rgba(0,0,0,0.8)">
                            access_time
                        </FontIcon>
                    </IconButton>
...
function getTransformedTableView(transform, key, pretext, description, allowtime, istime = false) {
    return {
        key: key,
...
function showTable(context) {
    if (context.data.length > 0) {
        ...
        if (n !== null && n.allbool) {
            t.push(getTransformedTableView(n.key === "" ? "ttrue" : "$('" + n.key + "') | ttrue",
                "ttruetable", "Time True", "Time that the stream spends in the true state", true, true));
        }
        return t;
    }
    return null;
