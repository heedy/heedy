...
export function getTimestamp(state) {
    if (state.customtimestamp === undefined || state.customtimestamp == false || state.timestamp === undefined) {
        return moment();
    }
...
class TimeChooser extends Component {
    ...
    render() {
        ...
        let customtimestamp = (state.customtimestamp !== undefined && state.customtimestamp == true);

        let timestamp = (state.timestamp !== undefined
            ? state.timestamp
            : moment());

        return (
            <div>
                <Toggle label="Custom Timestamp" labelPosition="right" toggled={customtimestamp} onToggle={(v, d) => {
                    ...
                    : (
                        <div>
                            <DateRangePicker startDate={state.timestamp} singleDatePicker={true} opens="left" timePicker={true} onEvent={(e, picker) => this.props.setState({timestamp: picker.startDate})}>
                            ...
                            <p>Make sure your timestamp is greater than all existing datapoints!</p>
                        </div>
                    ))}
            </div>
        );
