...
class AvatarIcon extends Component {
  ...
  render() {
    const { iconsrc, name, ...rest } = this.props;
    if (iconsrc !== undefined && iconsrc != "") {
      ...
      if (iconsrc.startsWith("material:")) {
        return (
          <Avatar
            {...rest}
            backgroundColor={stringToColor(iconsrc + name)}
            icon={
              <FontIcon className="material-icons">
                {iconsrc.substring("material:".length, iconsrc.length)}
              </FontIcon>
            }
          />
        );
    ...
    return (
      <Avatar {...rest} backgroundColor={stringToColor(name)}>
